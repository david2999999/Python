def main():
    class Omelet:
        ...
        An omlet object has the following interfaces
        get_kind() - returns a string with the type of omelet
        set_kind(kind) - returns the omelet to be the type named
        ...
        def __init__(self, kind="cheese"):
            """__init__ initializes the Omelet class to default to a cheese omelet"""
            self.set_kind(kind)
            return

        def __ingredients__(self):
            """Internal method to be called by a fridge or other objects that need to act on ingredients"""
